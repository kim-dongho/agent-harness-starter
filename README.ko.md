...
```bash
npx agent-harness-starter@latest init  # 첫 번째: Claude
npx agent-harness-starter@latest init  # 두 번째: Gemini
...
두 가지 트랙으로 사용할 수 있습니다.

### 이슈 기반 (Jira + Figma)

```
/start <이슈번호> → 이슈 조회 → Figma 분석 → 구현 계획 → 구현
/done              → 품질 게이트 → 커밋 → MR 생성
```

`/start`가 이슈 조회부터 구현 계획까지 한번에 수행합니다:

1. **이슈 조회** (`/fetch-issue`) — Jira API로 제목/설명/타입
2. **티켓 검증** — 필수 항목 (수락 기준, Figma 링크 등) 체크
3. **Figma 분석** (`/figma`) — 기획 + 디자인 MCP로 읽기
4. **브랜치 생성** (`/branch`) — 이슈 타입에 따라 `feature/`, `fix/` 등 prefix
5. **변경 대상 분석** — 코드베이스 탐색 + 복잡도 판단
6. **구현 계획** — 단계별 계획 제시 (사용자 확인 후 진행)
...
<p align="center">
  <img src="docs/harness-flow-detail.excalidraw.png" alt="Harness Detail Flow" width="400">
</p>
...
```bash
npm version patch && npm run publish:github   # 0.1.3 → 0.1.4 (버그 수정)
npm version minor && npm run publish:github   # 0.1.4 → 0.2.0 (기능 추가)
