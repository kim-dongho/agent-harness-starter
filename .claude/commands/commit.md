...
- **Scope**: `cli` / `scaffolder` / `generators` / `templates` / `scripts` / `docs` / `common`(루트 설정·공통) (복수 시 콤마 나열)
- **Description**:
  - 한국어, 소문자 시작, 마침표 없음
...
```
feat(scaffolder): 워크플로우 skills 템플릿 추가
fix(generators): 모노레포 Docker 포트 충돌 수정
refactor(cli): 프롬프트 플로우 분리
docs: README 업데이트
```
...
- `git add`는 사용자가 직접 실행 — 이 커맨드에서 `git add`를 실행하지 않음
- `.env` 파일 등 민감한 파일이 staged에 포함된 경우 경고 후 중단
- `index.ts` (자동 생성 파일)는 변경사항이 있을 때만 포함
